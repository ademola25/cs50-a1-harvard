...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # 3) Add a new sentence to knowledge base
            ...
            for dj in range(-1, 2):
                ...
                if 0 <= ni < self.height and 0 <= nj < self.width:
                    ...
                    if neighbor in self.mines:
                        # Count known mines among neighbors
        ...
        adjusted_count = count - known_mine_neighbors
        
        # Create new sentence with undetermined neighbors
        if neighbors:
            new_sentence = Sentence(neighbors, adjusted_count)
            self.knowledge.append(new_sentence)
        
        # 4 & 5) Iteratively update knowledge until no more changes
        changed = True
        while changed:
            changed = False
            
            # Mark any cells that can be determined from current knowledge
            for sentence in self.knowledge[:]:  # Use slice to avoid modifying during iteration
                # Mark known mines
